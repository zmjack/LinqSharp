...
internal class SearchFilter<T> : IFieldFilter<T>
{
    ...
    public QueryExpression<T> Filter(QueryHelper<T> h)
    {
        ...
        Expression BuildSearch(Expression field)
        {
            ...
            {
                return needConvert
                    ? Expression.Call(Expression.Call(field, MethodAccessor.Object.ToString), _search, search)
            ...
            if (searchConstants.Length > 1)
            {
                if (_useNot)
                {
                    ...
                    (
                        ...
                        select Expression.Not(Clause(exp))
                    ).ExpressionJoin(Expression.AndAlso)!);
                }
                else
                {
                    return CheckNullClause(
            ...
            else if (searchConstants.Length == 1)
            {
                ...
                {
                    return CheckNullClause(
                        Expression.Not(Clause(searchConstants[0]))
                    );
                }
                else
                {
