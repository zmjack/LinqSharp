...
public class RawQueryScope : Scope<RawQueryScope>
{
    private readonly List<IDisposable> _scopes = [];

    public RawQueryScope(DbContext context)
    {
        (context as IRowLockable)?.BeginIgnoreRowLock().Pipe(_scopes.Add);
        (context as ITimestampFormattable)?.BeginIgnoreTimestampFormat().Pipe(_scopes.Add);
